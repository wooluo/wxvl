...
原创 森林之家zbs
                    森林之家zbs  allby森林之家   2026-09-16 09:08  
  
## ThinkPHP是什么？  
  
ThinkPHP是国内用得最多的PHP开发框架，十个PHP网站里面估摸有三四个都是  
ThinkPHP  
搭的。  
...
还可以把  
ThinkPHP的icon图标保存到本地，直接用icon图标检索：  
...
去GitHub上搜ThinkphpGUI就可以下载了，支持各版本  
Thinkphp  
漏洞检测、命令执行、getshell：  
...
我把  
ThinkPHP  
各版本的通用漏洞全部都捋一遍，按版本线从  
2.x打头，再  
到8.x收尾，最后来一个文件上传getshell的实战案例。  
## 漏洞一：TP2.x RCE（preg_replace /e）  
  
从最老的版本说起：ThinkPHP 2.x  
早就不维护了，但还是会有些老站还在用，漏洞原因就是2.x在  
解析路由时用了preg_replace的/e模式。  
...
说白了就是**你想调什么PHP函数就能调什么**  
。phpinfo、system、file_put_contents，随便换。  
...
更要命的是，TP5默认开启debug模式，报错页面直接把**数据库账号密码**  
吐出来。  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/2wf8V0A7uBdzgbBEIibP1JbNRhqjeje4SeoemibTMXsQcjiahCnZ1HicFoS8QxLia0KbGSDy9wSO4Lc6N9OB8icqtA5cKLZEPjOkuaUYIk1K3udak/640?wx_fmt=png&from=appmsg "")  
...
![](https://mmbiz.qpic.cn/mmbiz_png/2wf8V0A7uBd6pkk7VzJw9TChAQBA9wZB4Yrrr1FgtqT4icspPcJWPgHc4jViaGXztdEOnKiboeyzpZU3Tvjf8CTCpJxvULTh8yawIib5lxGCFCo/640?wx_fmt=png&from=appmsg "")  
  
这里点到为止，打个phpinfo  
证明getshell成功：  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/2wf8V0A7uBdvcN4ibfE1FEsAnS0tWsDJZLleOWibrQZKQMow37awaZlt997Z9HNsuCfeOz7WfQ8gHCTtMoWxMN7CZIAiajKLicjmM0syO0tCjMc/640?wx_fmt=png&from=appmsg "")  
## 最后总结  
...
碰到  
ThinkPHP  
站点，按这个顺序来，基本都能拿下一个点。各版本打法都给师傅们捋清楚了，剩下的就是实战练。  
