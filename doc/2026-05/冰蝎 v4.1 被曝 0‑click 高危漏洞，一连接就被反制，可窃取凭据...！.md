...
s2cr3t
                    s2cr3t  Z2O安全攻防   2026-05-16 13:24  
  
本文是转载自《潇湘信安》，根据   
@  
s2cr3t   
师傅在冰蝎 (Behinder) 官方 Github 仓库提交的一个编号为 313 的安全漏洞报告，issues地址如下：  
...
客户端连接WebShell后，服务端返回的basicInfo  
字段为HTML格式，**未经任何过滤直接传入JavaFX WebView.loadContent()渲染**  
。  
  
即便代码中已执行setJavaScriptEnabled(false)  
禁用JavaScript，**WebKit 仍会自动解析 HTML/CSS 并加载外部资源**  
（图片、样式表、iframe、file 协议等）。  
...
其他版本  
：未逐一验证，**只要MainWindowController中在 WebView.loadContent(basicInfoStr) 且未做 HTML 过滤即受影响**  
...
搭建 Python 蜜罐，模拟冰蝎服务端协议（AES/ECB + Base64 + JSON），在 BasicInfo  
 响应中注入恶意 HTML：  
```
...
1. 服务端返回含恶意 HTML 的加密响应  
  
1. 客户端解密 → Base64解码 → 直接loadContent  
...
# NetNTLMv2 Hash（hashcat -m 5600 可直接破解）
...
1. WebKit 会自动请求 <img src>、<link href>、file:// 等资源  
  
1. basicInfoStr 从服务端完全可控，解密后直接渲染，无任何过滤  
  
1. file:// 会被转为 UNC 路径，触发系统级 SMB 认证  
...
    .replaceAll("<(?!br|/br|font|/font|b|/b|i|/i)[^>]*>", "")
    .replaceAll("(?i)(src|href|data|background|style)\\s*=", "");
...
infoArea.setText(basicInfoStr.replaceAll("<[^>]*>", ""));
```  
...
- 攻击前提：  
恶意服务端知道通信密码（蜜罐 / 反制场景天然满足）  
  
- 利用难度：  
极低，PoC 完整可用  
  
- 危害：  
可直接定位操作者真实身份、获取密码哈希、横向入侵内网  
...
![](https://mmbiz.qpic.cn/mmbiz_png/XOPdGZ2MYOeSsicAgIUNHtMib9a69NOWXw1A7mgRqqiat1SycQ0b6e5mBqC0pVJ3oicrQnCTh4gqMGiaKUPicTsUc4Tw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1#imgIndex=2 "")  
  
 还在等什么？赶紧点击下方名片开始学习吧   
![](https://mmbiz.qpic.cn/mmbiz_png/XOPdGZ2MYOeSsicAgIUNHtMib9a69NOWXw1A7mgRqqiat1SycQ0b6e5mBqC0pVJ3oicrQnCTh4gqMGiaKUPicTsUc4Tw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1#imgIndex=3 "")  
