...
1. 在 URL 参数中插入 XSS 测试代码，拼接生成恶意访问地址；  
  
1. 访问构造后的链接；  
  
1. 页面弹窗正常弹出，脚本运行生效，漏洞复现完成。  
...
1. 制作钓鱼链接，诱导用户泄露账号隐私数据；  
  
1. 借助漏洞发起后续渗透攻击，扩大安全影响。  
  
### 修复建议  
1. 对尖括号、引号、连接符等特殊字符进行拦截过滤；  
  
1. 页面输出数据时统一做 HTML 编码转义；  
  
1. 配置 CSP 安全策略，限制未知脚本运行；  
  
1. 校验外部传入参数格式，阻断恶意代码传入。  
...
1. 区分存储型与反射型差异，针对性书写报告内容；  
  
1. 上报时标注链接诱导风险，提升漏洞评定等级。  
