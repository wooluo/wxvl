...
原创 360漏洞研究院
                    360漏洞研究院  360漏洞研究院   2026-09-23 08:14  
  
致远协同管理软件 A8（Seeyon A8）是国内政企市场占有率较高的协同办公（OA）平台之一，广泛部署于政务、央企、金融、制造等行业的内网办公环境，通常承载公文流转、审批、门户、表单等核心业务。  
  
  
本次漏洞位于其**门户样式引擎**  
模块。该模块负责根据门户皮肤（skin）配置中的热点值（hotspot）动态生成区块样式表（SCSS → CSS），属于门户个性化能力的一部分。由于该能力在默认配置下对**任意可登录用户**  
开放，攻击面覆盖到每一个能登录 OA 的账号。  
...
该样式引擎属 A8 门户通用能力，**其他 A8 8.x 版本建议一并排查**  
（具体受影响范围待厂商确认）  
