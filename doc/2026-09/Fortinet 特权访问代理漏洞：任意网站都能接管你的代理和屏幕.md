...
第一步，把自己变成受信任服务器。扩展里有一个 webRequest 监听器，监听 https://*/api/v2/monitor/web-ui/state  
，只要收到请求，就把请求的主机名无条件加进“受信任服务器”列表。它不检查请求是谁发起的，也不管请求是否成功。  
...
第二步，未授权启动。扩展的 externally_connectable  
 配置允许外部连接，而且一个非 JWT 的 access token 不会触发校验失败，而是直接跳过验证。攻击者控制域名后，扩展就会从攻击者那里拉取整个会话配置。  
...
[1]   
https://www.fortiguard.com/psirt/FG-IR-26-168  
