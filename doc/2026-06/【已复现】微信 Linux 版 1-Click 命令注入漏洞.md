#  【已复现】微信 Linux 版 1-Click 命令注入漏洞  
原创 360漏洞研究院
                    360漏洞研究院  360漏洞研究院   2026-02-10 08:29  
...
微信 Linux 版是腾讯官方维护的桌面客户端，基于 Electron 框架开发。该客户端运行于主流 Linux 发行版，提供聊天、文件收发等核心功能。  
...
360 漏洞研究院已成功复现微信 Linux 版 1-Click 命令注入漏洞。攻击者向目标用户发送文件名中包含特殊字符构造的恶意指令文件，当受害者点击该文件时，程序在处理文件名过程中触发命令注入，执行攻击者构造的系统命令。  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/dZ7ia5iaWFzz8Epia28cwegUaicPt07yNZywiaSUgEu0V3icb8QibLm3Urrsfla4NI0ZapUhu2TFmEM0ghOJhdPZdvniacy30icbriaHTR650qVaOxvyo/640?wx_fmt=png&from=appmsg "")  
  
微信 Linux 版 1-Click 命令注入漏洞复现  
...
目前官方尚未发布微信 Linux 版的修复版本。建议用户在修复版本发布前采取以下临时防护措施：  
- 避免点击或打开文件名中包含异常特殊字符或可疑内容的文件，尤其是来源不明的文件。  
  
- 临时使用微信网页版（https://web.wechat.com/）进行必要的沟通与文件处理，以降低本地客户端被利用的风险。  
