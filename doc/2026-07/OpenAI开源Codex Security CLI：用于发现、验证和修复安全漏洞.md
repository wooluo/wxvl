...
- 扫仓库：  
对整个代码库做上下文分析，不再靠人肉 review 兜底。  
  
-  
 审 PR：  
代码还没合进主干，就能在拉取请求阶段把问题揪出来，而不是等上线后救火。  
  
- 进 CI/CD：  
一句话把 OPENAI_API_KEY挂上，安全检查自动跑在每次提交里。  
  
- 跟历史  
：扫描结果存在本地状态目录（不可写就用 CODEX_SECURITY_STATE_DIR指到别处），跨多次运行追踪同一批问题修没修。  
